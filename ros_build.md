...
cd ~/medrct_ws/dvrk_planning/docker
...
```bash
sh ~/medrct_ws/dvrk_planning/docker/exec_ros_noetic.sh
```
...
source /opt/ros/noetic/setup.bash
cd ~/medrct_ws/ambf
sudo apt update && xargs -a ~/medrct_ws/dvrk_planning/ambf-1.0-apt-requirements.txt sudo apt-get install -y
...
```bash
source ~/medrct_ws/ambf/build/devel/setup.bash
cd ~/medrct_ws/dvrk_planning/ros/medrct_ros_app/scripts
