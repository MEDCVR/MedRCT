...
public:
  ...
  {
    auto shared_input_stream = std::make_shared<SharedInputStream<T>>(
    ...
    cc_cfg.output_js_stream = std::make_shared<SharedOutputStream<JointState>>(
  ...
  void enableController(std::shared_ptr<ControllerInterface> ctrl, T input)
  {
