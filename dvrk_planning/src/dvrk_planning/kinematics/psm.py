...
from dvrk_planning.kinematics.utilities import *
...
def compute_fk(joint_pos, up_to_link = kinematics_data.num_links):
...
def compute_ik(T_7_0_mat):
    T_7_0 = convert_mat_to_frame(T_7_0_mat)

    # Pinch Joint
    T_PinchJoint_7 = Frame(Rotation.RPY(
        0, 0, 0), kinematics_data.L_yaw2ctrlpnt * Vector(0.0, 0.0, -1.0))
    ...
    T_PalmJoint_PinchJoint = Frame(Rotation.RPY(
        0, 0, 0), N_PalmJoint_PinchJoint * kinematics_data.L_pitch2yaw)
    ...
    j3 = insertion_depth + kinematics_data.L_tool2rcm_offset
