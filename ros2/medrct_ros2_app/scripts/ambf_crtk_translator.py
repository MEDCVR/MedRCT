...
class PSMTranslator:
    def __init__(self, crtk_namespace = '/PSM2', initial_jps = []):
        self._client = Client("ambf_crtk_translator")
        self._client.connect()
        time.sleep(2.0)
        print("object names", self._client.get_obj_names())
        # self.base_handle = self._client.get_obj_handle('psm1/baselinksimple')
        self.base_handle = self._client.get_obj_handle('psm/baselink')
        # self.base_handle.set_joint_types([JointType.REVOLUTE, JointType.REVOLUTE, JointType.PRISMATIC, JointType.REVOLUTE,
        ...
        # gm_js = JointState()
        # gm_js.position = 
        position = [0.4, -0.1, 0.1, 0.0, 0.0, self.yaw_pos]
        # print("Initial GM JP: ", gm_js.position)
        self.servo_jp(position)
    ...
    def update_jaw_links(self, yaw_pos, jaw_pos):
        ...
        self.base_handle.set_joint_pos(5, yaw_pos - jaw_pos)
        self.base_handle.set_joint_pos(6, -yaw_pos - jaw_pos)


        # self.base_handle.set_joint_pos('toolyawlink-toolgripper1link', yaw_pos - jaw_pos)
        # self.base_handle.set_joint_pos('toolyawlink-toolgripper2link', -yaw_pos - jaw_pos)
    ...
    def servo_jp(self, joint_positions):
        ...
        self.base_handle.set_joint_pos(0, joint_positions[0]) # yaw
        self.base_handle.set_joint_pos(1, joint_positions[1]) # pitch
        self.base_handle.set_joint_pos(2, joint_positions[2]) # insertion
        self.base_handle.set_joint_pos(3, joint_positions[3]) # roll
        self.base_handle.set_joint_pos(4, joint_positions[4]) # pitch
...
if __name__ == '__main__':
    ...
    # try:
    crtk_translator = PSMTranslator(crtk_namespace = "medrct/PSM2")
    rclpy.spin(node)
