 ...
 * @par Modified to remove console_bridge and replace with medrctlog.
 *  Modified to turn off clang format
 */
                                   ...
                                   const std::string& library_directory)
{
  ...
  if (ec)
  {
    ...
    //                         ec.message().c_str());
    medrctlog::debug("Failed to find or load library: %s with error: %s",
                            decorate(library_name, library_directory).c_str(),
                            ec.message().c_str());
    return false;
  }
