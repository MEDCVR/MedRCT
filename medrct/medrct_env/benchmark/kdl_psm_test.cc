...
int main(int argc, char** argv)
{
  ...
  // time(&after);
  // std::cout<<"elapsed time for FK: "<<difftime(after,before)<<" seconds for
  // "<<k<<" iterations"<<std::endl; std::cout<<"time per iteration for FK:
  // "<<difftime(after,before)/k<<" seconds."<<std::endl; time(&before);
  // for(k=0;k<1e7;k++)
  idsolver.CartToJnt(q, qdot, qdotdot, f, tau);
  // time(&after);
  // std::cout<<"elapsed time for ID: "<<difftime(after,before)<<" seconds for
  // "<<k<<" iterations"<<std::endl; std::cout<<"time per iteration for ID:
  // "<<difftime(after,before)/k<<" seconds."<<std::endl;
