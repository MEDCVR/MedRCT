...
{
inline Quaternion GetYamlQuaternionDefault(
    YAML::Node config, const std::string& key, const Quaternion& default_quat)
{
  if (!config[key])
    return default_quat;
  if (!config[key]["quaternion"])
    return default_quat;
    ...
    const YAML::Node& controller_config)
{
  ...
  std::string kin_tree_name =
      GetValueDefault<std::string>(kinematics_config, "tree", "");
  if (kin_tree_name != "")
  {
