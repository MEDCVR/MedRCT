...
public:
  ...
  {
    command_joint_stream = std::make_shared<IntraOutputStream<T>>(
        command_topic, command_topic + "_output_stream");
  }
  void publish(const T& data) { command_joint_stream->publish(data); }

private:
  std::shared_ptr<IntraOutputStream<T>> command_joint_stream;
};
...
public:
  ...
  {
    robot_command_js_stream = std::make_shared<IntraInputStream<JointState>>(
    ...
    robot_measured_js_stream = std::make_shared<IntraOutputStream<JointState>>(
...
private:
  ...
  std::shared_ptr<IntraOutputStream<JointState>> robot_measured_js_stream;
  std::shared_ptr<IntraInputStream<JointState>> robot_command_js_stream;
