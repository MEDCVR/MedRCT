...
public:
  ...
  //   cc_cfg.forward_kinematics =
  //   std::make_shared<XPrismaticForwardKinematics>();
  //   cc_cfg.inverse_kinematics =
  //   std::make_shared<XPrismaticInverseKinematics>(); return cc_cfg;
  // }
...
{
  medrctlog::set_level(medrctlog::level::debug); // Set global log level to
  StreamMaster::init();
  PipelineExecutor pe;
  ...
  Transform initial_input_tf = Transform::Identity();
