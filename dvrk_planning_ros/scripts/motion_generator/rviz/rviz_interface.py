...
def translator():
    while not rospy.is_shutdown():
        if len(jaw_position)>0 and len(curr_position)>0:
            dat = JointState()
            ...
            dat.position.append(curr_position [0])
            dat.position.append(curr_position [1])
            dat.position.append( - curr_position [1])
            dat.position.append(curr_position [1])
            dat.position.append( - curr_position [1])
            dat.position.append(curr_position [1])
            dat.position.append(curr_position [2])
            dat.position.append(curr_position [3])
            dat.position.append(curr_position [4])
            dat.position.append(curr_position [5])
            dat.position.append( - jaw_position [0])
            dat.position.append(jaw_position [0])
...
if __name__ == '__main__':
    ...
    rospy.Subscriber("/plot/trajectory/rviz",Float64MultiArray,trajectory_callback)
    ...
    Thread(target = translator).start()
    Thread(target = send_points).start() 
